...
WHERE `Titre` = :titre AND date.Date_evenement = :datee");
    ...
    $requete->execute();
    $requete->closeCursor();
    #revoie un message vers la page change_statut par le biais de la methode get
    $var = "Evenement mit en privé";
    header('location:change_statut.php?var=' . $var);
    exit();
} else {
    #revoie un message d'erreur vers la page change_statut par le biais de la methode get
